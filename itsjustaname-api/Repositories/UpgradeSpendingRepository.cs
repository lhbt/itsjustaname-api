...
namespace itsjustaname_api.Repositories
{
    public class UpgradeSpendingRepository : IUpgradeSpendingRepository
    {
        ...
        private IDictionary<string, IEnumerable<UpgradeModel>> UpgradePaths = new Dictionary<string, IEnumerable<UpgradeModel>>()
         {
        ...
        {
            if (UpgradePaths.ContainsKey(name))
            {
                return UpgradePaths[name];
            }
